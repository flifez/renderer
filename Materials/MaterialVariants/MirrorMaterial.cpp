...
Vec3 MirrorMaterial::shade(const Vec3& lightDirection, const Vec3& viewDirection, const Vec3& normal, const Vec3& lightColor, const Vec3& lightIntensity, const Vec3& ambient) const {
        ...
        return ambient * color + lightColor * lightIntensity * pow(std::max(0.0f, float(Vec3::dot(reflectionDirection, viewDirection))), 1 / (reflectivity * reflectivity)) * reflectivity * color;
    }
} // Raytracer